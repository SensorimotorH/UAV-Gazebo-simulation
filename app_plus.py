import asyncio
from fastapi import FastAPI
from pydantic import BaseModel
from sse_starlette import EventSourceResponse


from fastapi import Request
from contextlib import asynccontextmanager
from fastapi.responses import JSONResponse
from fastapi.staticfiles import StaticFiles
from fastapi.middleware.cors import CORSMiddleware
from agent_config import conf
import os

from logic import AgentProcess, create_logger

logger = create_logger(__name__)


class InputItem(BaseModel):
    input_str: str


agent: AgentProcess = AgentProcess()


@asynccontextmanager
async def lifespan(app: FastAPI):
    # 启动时的代码
    logger.info("Starting up background processes...")
    agent.start()

    yield

    # 关闭时的代码
    logger.info("Shutting down background processes...")
    agent.stop()


app = FastAPI(lifespan=lifespan)
origins = [
    "http://localhost:8000",
]

app.add_middleware(
    CORSMiddleware,
    allow_origins=origins,
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

app.mount("/chat", StaticFiles(directory="front-file/dist", html=True), name="static")
app.mount(
    "/static_file",
    StaticFiles(directory="static_file/img", html=False),
    name="static_file",
)


@app.get("/map/image")
async def get_image():
    image_dir = "static_file/img"  # 图片目录
    images = os.listdir(image_dir)
    if not images:
        return JSONResponse(content={"error": "No images found"}, status_code=404)

    # 获取最新的图片
    images.sort(
        key=lambda img: os.path.getmtime(os.path.join(image_dir, img)), reverse=True
    )
    latest_image = images[0]

    # 构造图片的URL
    image_url = f"/static_file/{latest_image}"
    return {"url": image_url}


@app.get("/map/object")
async def get_object():
    return agent.agent_object_output()


@app.get("/")
async def read_root():
    return {"Hello": "World"}


# app.include_router(
#     product_apirouter(conf),
#     prefix=conf.get("APIUrl", "prefix"),
#     tags=["api"],
#     responses={404: {"description": "Not found"}},
# )


@app.get(conf.get("LogicUrl", "agent_output_url"))
async def root(request: Request):
    async def event_generator(request: Request):
        while True:
            if await request.is_disconnected():
                print("连接已中断")
            else:
                data = None
                if agent is not None:
                    data = next(agent.agent_output())
                else:
                    data = "agent is None"
                    break
                yield {
                    "event": "message",
                    "retry": 15000,
                    "data": data,
                }
            await asyncio.sleep(0.5)

    g = event_generator(request)
    return EventSourceResponse(g)


@app.post(conf.get("LogicUrl", "agent_input_url"))
async def input(input_item: InputItem):
    return agent.agent_input(input_str=input_item.input_str.strip())


if __name__ == "__main__":
    import uvicorn

    config = uvicorn.Config(
        app=app,
        host="127.0.0.1",
        port=8000,
        workers=1,  # 使用单个worker，因为我们已经有了自己的进程管理
    )

    server = uvicorn.Server(config)
    try:
        server.run()
    except KeyboardInterrupt:
        print("stopping server")
